...
def solution(n: int, logs: list):
    ...
    connections = {str(i):{"direct": [], "indirect": []} for i in range(n)}
    ...
    for user, tmp_record in transaction_record.items():
        ...
        for amount, sender_records in tmp_record.items():
            if len(sender_records) > 1:
                ...
                if diff_minute < 10:
                    connections[sender_records[0][0]]["direct"].append(sender_records[1][0])
                    connections[sender_records[1][0]]["direct"].append(sender_records[0][0])
